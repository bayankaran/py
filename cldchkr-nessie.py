...
def get_mav_monthly(use_account_mav, apiKey, master_apiKey):
    ...
    for multi_account in use_account_mav:
        ...
        result = requests.get(base_url + 'billing.json/get_monthly_bill',
                              params={'access_key': access_key, 'cost_type': 'List', 'use_account': multi_account,
                                      'for_date': prev_month_yyyy}).json()

        logger.info('')
        logger.info(" Monthly Multi Account View Cost for " + multi_account + " using master access key '%s' ",
                    access_key[-8:])
        pp_json(result)
...
def get_accounts_v4(apiKey, master_apiKey, customerCode, useAccount):
    ...
    access_key = master_apiKey
    url = '{base_url}/account.json/get_accounts_v4?access_key={access_key}'.format(base_url=base_url,
...
def get_detailed_billing_with_grouping_v2_1(apiKey, master_apiKey, cc_account_id):
    ...
    result = requests.get(base_url + 'billing.json/get_detailed_billing_with_grouping_by_currency',
                          params={'access_key': access_key, 'start': start, 'end': end,
                                  'saved_filter_name': 'filter-monthly-account-costs',
                                  'use_cc_account_id': cc_account_id}).json()
...
if __name__ == '__main__':
    ...
    # super master api key, to be read from DynamoDB
    master_apiKey = ''
